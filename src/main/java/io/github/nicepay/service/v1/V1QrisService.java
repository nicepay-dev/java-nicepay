...
public class V1QrisService extends V1CommonService {

    private static LoggerPrint logger = new LoggerPrint();

    public static <S> S callRegistration(Qris data, NICEPay config) throws IOException {
        ...
        try {
            ...
            logger.logInfoV1("END CALL V1 REGISTRATION QRIS REQUEST");

        } catch (IOException ex) {
            logger.logErrorV1("IOException during V1 Registration QRIS Request: " + ex.getMessage());
            throw ex; // Re-throw IOException to ensure the caller is aware of the failure
        } catch (Exception ex) {
            logger.logErrorV1("Unexpected error during V1 Registration QRIS Request: " + ex.getMessage());
            ex.printStackTrace();
        }

        JsonObject jsonObject = JsonParser.parseString(resClient.toString()).getAsJsonObject();
        logger.logInfoResponseV1("Response Registration QRIS V1 :" + new GsonBuilder().setPrettyPrinting().create().toJson(jsonObject));
