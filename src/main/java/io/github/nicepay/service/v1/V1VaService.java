...
public class V1VaService extends V1CommonService {

    private static LoggerPrint logger = new LoggerPrint();

    public static <S> S callRegistration(VirtualAccount data, NICEPay config) throws IOException {

        logger.logInfoV1("START CALL V1 REGISTRATION VA REQUEST");
        Gson gson = new Gson();
        ...
        try {
            ...
            logger.logInfoV1("END CALL V1 REGISTRATION VA REQUEST");

        } catch (IOException ex) {
            logger.logErrorV1("IOException during V1 Registration VA Request: " + ex.getMessage());
            throw ex; // Re-throw IOException to ensure the caller is aware of the failure
        } catch (Exception ex) {
            logger.logErrorV1("Unexpected error during V1 Registration VA Request: " + ex.getMessage());
            ex.printStackTrace();
        }

        JsonObject jsonObject = JsonParser.parseString(resClient.toString()).getAsJsonObject();
        logger.logInfoResponseV1("Response Registration VA V1 :" + new GsonBuilder().setPrettyPrinting().create().toJson(jsonObject));
