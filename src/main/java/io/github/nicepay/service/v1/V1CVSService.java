...
public class V1CVSService extends V1CommonService {

    private static LoggerPrint logger = new LoggerPrint();

    public static <S> S callRegistration(ConvenienceStore data, NICEPay config) throws IOException {

        logger.logInfoV1("START CALL V1 REGISTRATION CVS REQUEST");
        Gson gson = new Gson();
        ...
        try {
            ...
            logger.logInfoV1("END CALL V1 REGISTRATION CVS REQUEST");

        } catch (IOException ex) {
            logger.logErrorV1("IOException during V1 Registration CVS Request: " + ex.getMessage());
            throw ex; // Re-throw IOException to ensure the caller is aware of the failure
        } catch (Exception ex) {
            logger.logErrorV1("Unexpected error during V1 Registration CVS Request: " + ex.getMessage());
            ex.printStackTrace();
        }

        JsonObject jsonObject = JsonParser.parseString(resClient.toString()).getAsJsonObject();
        logger.logInfoResponseV1("Response Registration CVS V1 :" + new GsonBuilder().setPrettyPrinting().create().toJson(jsonObject));
