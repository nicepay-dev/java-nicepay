...
public class V2QrisService extends V2CommonService {

    private static final Logger LOGGER = LoggerFactory.getLogger("[V2 - Qris]");

    public static <S> S callV2QrisRegistration(Qris data, NICEPay config) throws IOException {

        LOGGER.info(LoggerPrint.LOG_DEFAULT, "START CALL V2 QRIS REGISTRATION");
...
//        Update merchant token
        ...
        try {
            ...
            LOGGER.info(LoggerPrint.LOG_RESPONSE, "Response register QRIS transaction :" + new GsonBuilder().setPrettyPrinting().create().toJson(jsonObject));
            LOGGER.info(LoggerPrint.LOG_DEFAULT, "END CALL V2 QRIS REGISTRATION");

        } catch (Exception ex) {
            LOGGER.error(LoggerPrint.LOG_ERROR, "QRIS transaction registration failed :");
            ex.printStackTrace();
        }
